  ...
  computed: {
    errors() {
      ...
      return this.validation.$flattenParams().reduce((errors, rule) => {
        ...
        } else if(isInvalid && rule.name === 'required') {
...
<template>
  ...
  >
    <Label
      ...
      :is-optional="!validation || (validation && !validation.$params.required && !validation.value) || (validation && !validation.$params.required && !validation.value.$params.required)"
